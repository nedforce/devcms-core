class Admin::NewsViewerItemsController < Admin::AdminController
  ...
  def index
    @items = @news_viewer.news_viewer_items.includes(:news_item).order(:position)
  ...
  def available_news_items
    respond_to do |format|
      ...
      format.xml do
        ...
        @available_news_items       = @news_archive.news_items.newest.includes(:node).reorder('nodes.publication_start_date DESC').limit(@page_limit).offset(@page_limit*(@current_page-1))
      end
    end
  end

  def create
    @news_viewer_item = @news_viewer.news_viewer_items.find_or_initialize_by(news_item_id: NewsItem.find(params[:news_item_id]).id)
    respond_to do |format|
  ...
  def delete_news_item
    @item = @news_viewer.news_viewer_items.where(news_item_id: params[:news_item_id]).first
    respond_to do |format|
  ...
  def update_positions
    ...
    params[:items].each_with_index{ |id, index| NewsViewerItem.where(id: id).update_all(position: index) } if params[:items].present?
