...
class NewsViewer < ActiveRecord::Base
  ...
  def last_updated_at
    image = news_items.newest.accessible.first(order: 'news_viewer_items.position, nodes.publication_start_date DESC').node.children.with_content_type('Image').first rescue nil
