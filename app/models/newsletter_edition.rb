# This model is used to represent a newsletter edition item. A newsletter
# edition is contained within a newsletter archive, which in turn is represented
# by the +NewsletterArchive+ model. A newsletter edition can contain multiple
# +NewsItem+ objects. It has specified +acts_as_content_node+ from
...
# * +newsletter_archive+ - The newsletter archive that this newsletter edition
#                          belongs to.
...
class NewsletterEdition < ActiveRecord::Base
  ...
  # A +NewsletterEdition+ has many +NewsletterEditionItem+ objects and many
  # items through +NewsletterEditionItem+.
  ...
  def items
    newsletter_edition_items.all(:order => 'position ASC').map(&:item)
  end

  # Returns the number of items belonging to this newsletter edition.
  def items_count
    newsletter_edition_items.count
  end

  # Adds items to a +NewsletterEdition+, which must be a +Page+ or a +NewsItem+.
  # Old associations are removed first.
  #
  # Parameters: An array containing node IDs. The order of the items in the
  # array determines the positions of the items in the newsletter edition.
  def associate_items(items)
    # Use delete_all instead of destroy_all (quicker).
    NewsletterEditionItem.delete_all "newsletter_edition_id = #{id}"

    # Add the new items.
    if items
      items.each_index do |index|
        newsletter_edition_items.create(:item => Node.find(items.at(index)).content, :position => index)
      end
    end
  end
  ...
  def remove_associated_content
    ...
    newsletter_edition_queues.destroy_all
  end
end
