module NodeExtensions::UrlAliasing
  ...
  def parent_url_alias
    parent.url_alias if parent && !parent.is_global_frontpage? && !parent.root? && parent.sub_content_type != 'Site'
  end

  def generate_unique_url_alias
    uniqify_url_alias(generate_url_alias[0..(MAXIMUM_URL_ALIAS_LENGTH - 6)])
  end

  def generate_unique_custom_url_alias
    uniqify_url_alias(generate_custom_url_alias[0..(MAXIMUM_URL_ALIAS_LENGTH - 6)])
  end
  ...
  def set_custom_url_alias
    self.custom_url_alias = (custom_url_suffix.present? ? generate_unique_custom_url_alias : nil) if custom_url_suffix_changed?
  end
  ...
  def should_not_have_reserved_url_alias
    errors.add(:url_alias, :reserved_url_alias) if self.class.url_alias_reserved?(url_alias)
  end

  # Prevents saving this node when the URL alias contains reserved words.
  def should_not_have_reserved_custom_url_alias
    errors.add(:url_alias, :reserved_custom_url_alias) if self.class.url_alias_reserved?(custom_url_alias)
  end
  ...
  def nodes_to_exclude
    containing_site.children.with_content_type('Site') if containing_site.root?
  end
end
