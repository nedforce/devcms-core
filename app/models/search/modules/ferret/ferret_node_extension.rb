module Search::Modules::Ferret::FerretNodeExtension
  ...
  def acts_as_searchable
    ...
    # Index any approved content, which has not been hidden or excluded from
    # indexing.
  ...
  module InstanceMethods
    ...
    def path_to_index
      "/#{self_and_ancestors.map(&:id).join('/')}/"
    end

    def ancestry_to_index
      "XX#{ancestry.gsub(/\//, 'X')}X"
    end
    ...
    def is_hidden_to_index
      ...
      Settler[:ferret_exclude_node_ids].split(',').each do |nid|
        hidden_from_index = hidden_from_index || path_ids.include?(nid.to_i)
    ...
    def zipcodes_to_index
      content.addresses.map(&:postal_code) if content_type == 'Permit'
    end

    def updated_at_to_index
      updated_at.strftime(Node::INDEX_DATETIME_FORMAT)
    end
    ...
    def publication_start_date_to_index
      publication_start_date.strftime(Node::INDEX_DATETIME_FORMAT)
    end

    # Returns the publication end date for indexing
    def publication_end_date_to_index
      publication_end_date.nil? ? 'none' : publication_end_date.strftime(Node::INDEX_DATETIME_FORMAT)
    end
    ...
    def dynamic_boost
      ...
      # Content that has a nil body but a matching title will be ranked very
      # high because the hits/size ratio is high. Normally however nil body
      # content isn't all that interesting, so decrease its boost a bit.
      if content_tokens_to_index.length == 0
        boost -= 0.2
      end

      boost *= calculate_dynamic_boost_date_factor

      boost >= 0.1 ? boost : 0.1
    end

    # Calculates a negative boost factor based on whether the "content date" of
    # the item is "past", "current" or "future".
    def calculate_dynamic_boost_date_factor
      # Date only matters when the content is a NewsItem, NewsletterEdition,
      # CalendarItem or Meeting instance.
      unless [NewsItem, NewsletterEdition, CalendarItem, Meeting].any? { |type| content.is_a?(type) }
        ...
        # If it's a future or past item, we apply different (negative) boost
        # factors.
