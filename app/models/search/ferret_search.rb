class Search::FerretSearch
  def self.search(query, page, page_size, user, top_node, options)
    ...
    paginating_ferret_search({ q: query, page_size: page_size, current: page, limit: 250 }.merge(options))
  end
  ...
  def self.ferret_configuration
    ...
    Rails.logger.warn "[#{Time.now}][FerretSearcher] No ferret configuration directives found in treehouse.rb! Should this module be enabled? Using default values instead..." if config.nil?
