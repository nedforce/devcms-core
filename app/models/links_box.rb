...
class LinksBox < ActiveRecord::Base
  ...
  def last_updated_at
    ([self.node.self_and_descendants.accessible.maximum('nodes.updated_at')] + self.node.children.with_content_type("InternalLink").collect {|link_node| link_node.content.linked_node.update_at }).compact.max
  end
