# A content copy is a content node that represents a copy of another content
# node, i.e. it acts as a symbolic link. It has specified +acts_as_content_node+
# from Acts::ContentNode::ClassMethods.
#
# To prevent circular references, it is not allowed that the linked Node
# instance (as identified by +copied_node+) is associated with a ContentCopy
# content node.
#
...
# * Requires the linked Node to not be associated with a ContentCopy content
#   node.
# * Requires the linked Node to be associated with a content node that allows to
#   be copied.
...
class ContentCopy < ActiveRecord::Base
  ...
  def title
    copied_node.content.title
  end

  def title_changed?
    copied_node.content.title_changed?
  end
  ...
  def icon_filename
    copied_node.content.icon_filename
  end

  # Returns the content class of the copied node.
  def copied_content_class
    copied_node.blank? ? self.class : copied_node.content_class
  end
  ...
  def respond_to?(*args)
    if copied_node.present? && copied_node.content.respond_to?(*args)
      true
    else
      super
    end
  ...
  def copy_publication_and_expiration_dates
    return if copied_node.blank?

    self.expires_on             = copied_node.expires_on
    self.publication_start_date = copied_node.publication_start_date
    self.publication_end_date   = copied_node.publication_end_date
  end

  # To prevent circular references, we must ensure that the copied Node instance
  # (as identified by +copied_node+) is not associated with a ContentCopy
  # content node.
  def ensure_copied_node_is_not_associated_with_a_content_copy_content_node
    if copied_node && copied_node.content.is_a?(ContentCopy)
      errors.add(:base, :copied_content_cannot_be_content_copy)
    end
  end

  # Ensure the copied node is associated with a content node that allows to be
  # copied.
  def ensure_copied_node_is_associated_with_a_copyable_content_node
    if copied_node && !copied_node.content_type_configuration[:copyable]
      errors.add(:base, :copied_content_must_be_copyable)
    end
  end
end
