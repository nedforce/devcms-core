...
# * +email_address+ - The email address to which contact requests are send;
#                     typically an email address of an editor or administrator.
# * +description_before_contact_fields+ - The description of the contact form
#                                   (printed before the actual contact fields).
# * +description_after_contact_fields+ - The description of the contact form
#                                    (printed after the actual contact fields).
...
class ContactForm < ActiveRecord::Base
  # Constants for different send methods. Used in the send_method column.
  SEND_METHOD_MAIL     = 0
  ...
  # Ensure the +ContactFormFields+ are valid, build on creation of the
  # +ContactForm+, and updated and deleted when necessary.
  ...
  # Virtual attribute to keep track of +ContactFormField+ objects that need to
  # be deleted.
  ...
  # Returns an array with the ids of the +ContactFormField+ objects that are
  # obligatory.
  ...
  def email_address_field_ids
    contact_form_fields.field_type('email_address').map(&:id)
  end
      ...
      @contact_form_fields_before_save.each do |options|
        ...
        if contact_form_field_id && !new_record?
          if delete_contact_form_field == '1'
            ...
            inst = contact_form_fields.select { |s| s.id == contact_form_field_id.to_i }.first
  ...
  def destroy_deleted_contact_form_fields
    deleted_contact_form_fields.each do |ids|
