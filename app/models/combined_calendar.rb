...
# objects. It has specified +acts_as_content_node+ from
...
class CombinedCalendar < ActiveRecord::Base
  ...
  def calendar_items
    ...
    containing_site = node.containing_site
    ...
    accessible_calendar_node_child_ancestries = containing_site.descendants.accessible.exclude_subtrees_of(nodes_to_exclude).with_content_type('Calendar').map(&:child_ancestry)
  ...
  def last_updated_at
    [calendar_items.accessible.maximum('nodes.created_at'), updated_at].compact.max
  end
