...
class NewsletterEditionMailerWorker
  ...
  def publish_newsletter_edition(newsletter_edition)
    logger.info "#{Time.now}: #{newsletter_edition.id}: Started."
    queue = get_queue_for(newsletter_edition)
    if queue
      logger.info "#{Time.now}: #{newsletter_edition.id}: Now sending..."
      queue.each { |queued_subscription| send_queued_subscription(queued_subscription) }
      logger.info "#{Time.now}: #{newsletter_edition.id}: Done sending."
  ...
  def send_queued_subscription(queued_subscription)
    ...
    if queued_subscription.destroy
      begin
        ...
        logger.info "#{Time.now}: #{queued_subscription.newsletter_edition.id}: Could not send to user #{queued_subscription.user.id}."
        logger.info "#{Time.now}: #{queued_subscription.newsletter_edition.id}: #{e.message}."
      end
    else
      logger.info "#{Time.now}: #{queued_subscription.newsletter_edition.id}: Could not remove user #{queued_subscription.user.id} from the queue, postponing!"
    end
  end
  ...
  def build_queue_for(newsletter_edition)
    NewsletterEditionQueue.transaction do
      ...
      logger.info "#{Time.now}: #{newsletter_edition.id}: Queueing #{subscribers.size} subscriptions."
  ...
  def get_queue_for(newsletter_edition)
    ...
    unless queue
      logger.info "#{Time.now}: #{newsletter_edition.id}: Could not resume or build queue, aborting."
      return false
    end
