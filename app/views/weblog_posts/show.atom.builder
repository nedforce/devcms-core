atom_feed(:schema_date => '2008-05-19', :url => content_node_url(@weblog_post.node, :format => 'atom'), :language => 'nl', :root_url => content_node_url(@weblog_post.node)) do |feed|
  ...
  for comment in @comments
    feed.entry(comment, :updated => comment.updated_at, :published => comment.created_at, :url => content_node_url(@weblog_post.node, :anchor => "comment#{comment.id.to_s}")) do |entry|
