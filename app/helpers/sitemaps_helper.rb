module SitemapsHelper
  ...
  def create_sitemap(levels = 6)
    items = current_site.children.accessible.is_public.shown_in_menu.include_content.order(:position).map(&:content)
  ...
  def create_subitem(item, levels)
    ...
    subitems = item.node.children.accessible.is_public.shown_in_menu.include_content.order(:position).map(&:content)
