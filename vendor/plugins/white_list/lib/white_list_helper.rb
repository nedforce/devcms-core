module WhiteListHelper
  ...
  def white_list(html, options = {}, &block)
    ...
    options = {:override_defaults => false}.merge(options)
    
    attrs   = Set.new(options[:attributes])
    tags    = Set.new(options[:tags])
    
    unless options[:override_defaults]
      attrs.merge white_listed_attributes
      tags.merge white_listed_tags
    end
          ...
          when HTML::Tag
            node.attributes.keys.each do |attr_name|
              ...
              if !attrs.include?(attr_name) || (protocol_attributes.include?(attr_name) && contains_bad_protocols?(value))
                ...
                node.attributes[attr_name] = ((attr_name == 'href') && !options[:escape_href]) ? value : CGI::escapeHTML(value) 
              end
