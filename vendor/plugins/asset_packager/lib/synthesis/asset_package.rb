module Synthesis
  class AssetPackage
    ...
    # instance methods
    attr_accessor :asset_type, :target, :target_dir, :sources, :plugin
    ...
    def initialize(asset_type, package_hash)
      ...
      if package_hash[package_hash.keys.first].first.is_a?(Hash)
        @plugin = package_hash[package_hash.keys.first].first["plugin"]
        @sources = package_hash[package_hash.keys.first][1..-1]
      else
        @sources = package_hash[package_hash.keys.first]
      end
      @asset_type = asset_type
      @asset_path = ($asset_base_path ? "#{$asset_base_path}/" : "#{RAILS_ROOT}/public/")
      @asset_path << "plugin_assets/#{@plugin}/" if  @plugin
      @asset_path << "#{@asset_type}#{@target_dir.gsub(/^(.+)$/, '/\1')}"
