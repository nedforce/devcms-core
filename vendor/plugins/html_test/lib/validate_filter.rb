  ...
  module Test
    class ValidateFilter
      ...
      def validate_page
        ...
        assert_validates(validators, response.body.strip, url, :verbose => false)
      ...
      def should_validate?
        response.status =~ /200/ &&
        (response.headers['Content-Type'] =~ /text\/html/i && response.body =~ /<html/) # Do not validate HTML fragments as they'll always be invalid.
      end      
    end
  end
end
