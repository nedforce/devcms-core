...
SXE.insertElement = function(element_name, value) {
	var elm = SXE.inst.selection.getNode();

    tinyMCEPopup.execCommand('mceBeginUndoLevel');
	if (elm.tagName != 'ABBR' && elm.tagName != "abbr" ) {
		var s = value.length > 0 ? value : SXE.inst.selection.getContent({format: 'text'});
		if(s.length > 0) {
		...
		setAllCommonAttribs(elm);
        tinymce.isIE ? elm.innerText = value : elm.textContent = value ;
