...
class SitemapsControllerTest < ActionController::TestCase
  ...
  test 'should not contain unpublished news items in atom' do
    ...
    assert_response :success
    assert_nil(assigns(:nodes).to_a.reject! { |node| !node.content.respond_to?(:title) || node.content.title == 'Nog niet beschikbaar' }, 'Not yet published items should not be shown')
    assert_nil(assigns(:nodes).to_a.reject! { |node| !node.content.respond_to?(:title) || node.content.title == 'Niet meer beschikbaar' }, 'No longer published items should not be shown')
    assert_not_nil(assigns(:nodes).to_a.reject! { |node| !node.content.respond_to?(:title) || node.content.title == 'Beschikbaar' }, 'Published items should not have been deleted')
  end
  ...
  test 'should not contain hidden content in atom' do
    get :changes, format: 'atom'
    assert_response :success
    assert_nil assigns(:nodes).to_a.reject! { |n| !n.visible? }
  end
