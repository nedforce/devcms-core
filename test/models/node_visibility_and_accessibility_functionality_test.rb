...
class NodeVisibilityAndAccessibilityFunctionalityTest < ActiveSupport::TestCase
  ...
  def test_private_scope
    node = create_node
    assert_nil Node.is_private.find_by_id(node.id)
    node.update_attribute(:private, true)
    assert_not_nil Node.is_private.find_by_id(node.id)
  end

  def test_public_scope
    node = create_node
    assert_not_nil Node.is_public.find_by_id(node.id)
    node.update_attribute(:private, true)
    assert_nil Node.is_public.find_by_id(node.id)
  end
