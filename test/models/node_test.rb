...
class NodeTest < ActiveSupport::TestCase
  ...
  test 'should not destroy root node' do
    assert_raise ActiveRecord::ActiveRecordError do
  ...
  test 'should destroy associated content' do
  ...
  def test_should_return_hash_for_tree_node
    ...
    assert tree_node.key?(:id)
    assert tree_node.key?(:text)
  end
  ...
  def test_should_create_version_and_check_accessibility_flow
    ...
    assert Node.accessible.find(page.node.id)
  ...
  def test_should_sort_children_ascending_by_content_title
    ...
    assert_equal %w(aaa BBB ccc), node.children.map { |n| n.content.content_title }
  end

  test 'should sort children descending by created at' do
    node = nodes(:node_for_sorting)
  ...
  def test_node_should_be_expandable_if_user_has_a_role_on_a_descendant
    assert @root_node.is_expandable_for_user?(@editor)
  end
  ...
  def test_determine_content_date_should_use_publication_start_date_for_news_items_and_newsletter_editions
    ...
    [
      ...
      create_newsletter_edition(publication_start_date: publication_start_date)
    ].each do |item|
  ...
  test 'changes should not include unpublished new items' do
    ...
    create_news_item(publication_start_date: 2.days.from_now, publication_end_date: 3.days.from_now, title: 'Nog niet beschikbaar')
    create_news_item(publication_start_date: 2.days.ago,      publication_end_date: 1.day.ago,       title: 'Niet meer beschikbaar')
    changes = nodes(:devcms_news_node).last_changes(:all).to_a
  ...
  def test_changes_should_not_include_feeds
    assert_not_nil Node.where(content_type: 'Feed'), 'Should have at least one feed to test for exclusion'
  ...
  def test_changes_should_only_include_accessible_children
    assert_nil @root_node.last_changes(:all).to_a.reject! { |n| !n.publishable? }
  end
  ...
  test 'should return self for changes' do
    create_news_item(publication_start_date: 2.days.ago, publication_end_date: 1.day.from_now, title: 'Beschikbaar')
  ...
  test 'should reduce number of hits on all nodes' do
    page_node_1 = create_page.node
    page_node_2 = create_page.node
    page_node_3 = create_page.node

    page_node_1.update_attribute(:hits, 3000)
    page_node_2.update_attribute(:hits, 1000)
    page_node_3.update_attribute(:hits, 2000)

    Node.reduce_hit_count

    assert_equal 300, page_node_1.reload.hits
    assert_equal 100, page_node_2.reload.hits
    assert_equal 200, page_node_3.reload.hits
  end

  test 'should accept tag' do
  ...
  test 'should return containing site' do
    assert_equal @root_node,                    nodes(:help_page_node).containing_site
    ...
    assert_equal @root_node,                    @root_node.containing_site
  end
  ...
  test 'should not update positions in subtree on move' do
  ...
  def create_news_item(options = {})
    NewsItem.create({
      ...
      publication_end_date: 1.day.from_now
    }.merge(options))
  end
  ...
  def create_newsletter_edition(options = {})
    NewsletterEdition.create({
      ...
      publication_start_date: 1.hour.from_now
    }.merge(options))
  end
  ...
  def create_editor_content_node(options = {})
    Page.create({
      user: users(:editor),
      parent: nodes(:editor_section_node),
      title: 'foo',
      preamble: 'xuu',
      body: 'bar',
