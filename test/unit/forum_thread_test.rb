...
class ForumThreadTest < ActiveSupport::TestCase
  ...
  def test_last_update_date_should_return_creation_date_of_last_created_forum_post
    post = @bewoners_forum_thread_one.forum_posts.create(:body => 'foobar', :user => users(:henk), :created_at => Time.now + 4.hours)
    @bewoners_forum_thread_one.forum_posts.create(       :body => 'foobar', :user => users(:henk), :created_at => Time.now + 3.hours)
    @bewoners_forum_thread_one.forum_posts.create(       :body => 'foobar', :user => users(:henk), :created_at => Time.now + 2.hours)
  ...
  def test_start_post_should_return_first_created_post
    ...
    forum_threads.each do |thread|
      assert_equal thread.start_post, thread.forum_posts.first(:order => 'created_at ASC')
    end
  end
